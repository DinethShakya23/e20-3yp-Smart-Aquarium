...
def extract_image_patch(image, bbox, patch_shape):
    ...
    bbox[2:] += bbox[:2]
    bbox = bbox.astype(np.int32)  # Change np.int to np.int32
...
class ImageEncoder(object):
    def __init__(self, checkpoint_filename, input_name="images", output_name="features"):
        # Use tf.compat.v1 for compatibility with TensorFlow 1.x code
        self.session = tf.compat.v1.Session()
        with tf.io.gfile.GFile(checkpoint_filename, "rb") as file_handle:
            graph_def = tf.compat.v1.GraphDef()  # Use tf.compat.v1.GraphDef for TensorFlow 1.x code
